...
public class BookingPane extends BasicPane {
	...
	private void fillNameList() {
		...
		db.excecuteSQLquery(sql, nameListModel, "moviename");
	...
	private void fillDateList(String movieName) {
		...
		String sql = "select showdate from performance where moviename = '"
				+ movieName + "';";
		db.excecuteSQLquery(sql, dateListModel, "showdate");
	}

	private String getTheater(String movieName) {
		String temp = db.excecuteSqlquery(movieName, "theatername");
		return temp;
	}

	private String getSeatsAvailable(String mName, String tName) {
		/* --- insert own code here --- */
		String temp = db.excecuteSqlseatsQuery(mName, tName);
		return temp;
	}

	private void bookSeat(String uName, String showdate, String movieName, String tName) {
		db.excecuteSqlbooking(uName, showdate, movieName, tName);
	}
	...
	class DateSelectionListener implements ListSelectionListener {
		...
		public void valueChanged(ListSelectionEvent e) {
			...
			String theater = getTheater(movieName);
			fields[2].setText(theater);
			String seats = getSeatsAvailable(movieName, theater);
			fields[3].setText(seats);
		}
	}
	...
	class ActionHandler implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			bookSeat(userName, date, movieName, getTheater(movieName));
		}
	}
}
